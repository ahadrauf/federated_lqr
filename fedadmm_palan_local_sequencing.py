...
if __name__ == "__main__":
    ...
    for traj in traj_range:
        ...
        for seed in seed_range:
            ...
            if traj > 1:
                ...
                prevfedP, prevfedQ, prevfedR = out_fedadmm_aggregate[-1][1:]
            else:
                prevP, prevQ, prevR = None, None, None
                prevfedP, prevfedQ, prevfedR = None, None, None
            ...
            for m in range(M):
                ...
                def Lfed_QR(K, Q, R):
                    return sum([cp.sum_squares(K@x - u) for x, u in zip(xs, us)]) + cp.sum_squares(Q - prevfedQ) + \
                           cp.sum_squares(R - prevfedR)


                if traj == 1:
                    KfedadmmQR, PfedadmmQR, QfedadmmQR, RfedadmmQR = \
                        policy_fitting_with_a_kalman_constraint_extra(L, r, xs, us, cont.A, cont.B,
                                                                      P0=prevfedP, Q0=prevfedQ, R0=prevfedR,
                ...
                # seed = np.random.randint(0, 1e6)
                # cost_lr = cont.simulate(x0, N, K=Klr, seed=seed, add_noise=True)[2][1]
                cost_lr = cont.loss_imitation_learning(x0, N, xs, us, K=Klr)
                if np.linalg.norm(Qadmm) == np.inf:
                    cost_admm = np.nan
                else:
                    # cost_admm = cont.simulate(x0, N, Q=Qadmm, R=Radmm, seed=seed, add_noise=True)[2][1]
                    cost_admm = cont.loss_imitation_learning(x0, N, xs, us, Q=Qadmm, R=Radmm)
                if np.linalg.norm(QfedadmmQR) == np.inf:
                    cost_fedadmmQR = np.nan
                else:
                    # cost_fedadmmQR = cont.simulate(x0, N, Q=QfedadmmQR, R=RfedadmmQR, seed=seed, add_noise=True)[2][1]
                    cost_fedadmmQR = cont.loss_imitation_learning(x0, N, xs, us, Q=QfedadmmQR, R=RfedadmmQR)
        ...
        Kavg = np.nanmean([K for _, _, K, P, Q, R in out_admm[-nseed*M:]], axis=0)
        Pavg = np.nanmean([P for _, _, K, P, Q, R in out_admm[-nseed*M:]], axis=0)
        Qavg = np.nanmean([Q for _, _, K, P, Q, R in out_admm[-nseed*M:]], axis=0)
        Ravg = np.nanmean([R for _, _, K, P, Q, R in out_admm[-nseed*M:]], axis=0)

        Kfedavg = np.nanmean([K for _, _, K, P, Q, R in out_fedadmmQR[-nseed*M:]], axis=0)
        Pfedavg = np.nanmean([P for _, _, K, P, Q, R in out_fedadmmQR[-nseed*M:]], axis=0)
        Qfedavg = np.nanmean([Q for _, _, K, P, Q, R in out_fedadmmQR[-nseed*M:]], axis=0)
        Rfedavg = np.nanmean([R for _, _, K, P, Q, R in out_fedadmmQR[-nseed*M:]], axis=0)
        # out_fedaddmmK.append((traj, Kavg))
        out_admm_aggregate.append((traj, Pavg, Qavg, Ravg))
        out_fedadmm_aggregate.append((traj, Pfedavg, Qfedavg, Rfedavg))
        ...
        np.save('data/' + timestamp + "_fedadmm_palan_local_sequencing.npy", [costs_lr_vsN, std_costs_lr_vsN,
                                                       costs_admm_vsN, std_costs_admm_vsN,
                                                       costs_fedadmmQR_vsN, std_costs_fedadmmQR_vsN,
                                                       lossK_lr_vsN,
                                                       lossK_admm_vsN, lossQ_admm_vsN, lossR_admm_vsN,
        ...
        if traj == 1:
            ...
            plt.savefig("figures/" + timestamp + "_fedadmm_palan_local_sequencing_trajectories.png")
            plt.savefig("figures/" + timestamp + "_fedadmm_palan_local_sequencing_trajectories.pdf")
            # plt.show()
    ...
    plt.savefig("figures/" + timestamp + "_fedadmm_palan_local_sequencing.png", bbox_inches='tight')
    plt.savefig("figures/" + timestamp + "_fedadmm_palan_local_sequencing.pdf", bbox_inches='tight')
    print(timestamp)
    # plt.show()
