...
def policy_fitting_with_a_kalman_constraint(L, r, xs, us_observed, A, B, n_random=5, niter=50, rho=1):
    ...
    for _ in range(n_random):
        ...
        K = _policy_fitting_with_a_kalman_constraint(L, r, xs, us_observed, A, B, P.T@P, Q.T@Q, R.T@R, niter=niter,
                                                     rho=rho)
        L_K = evaluate_L(K)
        if L_K < best_L:
            best_L = L_K
...
def policy_fitting_with_a_kalman_constraint_local(L, r, xs, us_observed, A, B, n_random=5, niter=50, rho=1):
    """
    Wrapper around _policy_fitting_with_a_kalman_constraint.
    """
    n, m = B.shape
...
def policy_fitting_with_a_kalman_constraint_federated_personalized(L, r, xs, us_observed, A, B, n_random=5, niter=50, rho=1):
    """
    Wrapper around _policy_fitting_with_a_kalman_constraint.
    """
    n, m = B.shape
...
#             best_K = K
    ...
    return best_K / (n_random + 1)


def policy_fitting_with_a_kalman_constraint_federated_personalized_feedback(L, r, xs, us_observed, A, B, P0=None, Q0=None, R0=None, K0=None, n_random=5, niter=50, rho=1):
    """
    Wrapper around _policy_fitting_with_a_kalman_constraint.
    """
    n, m = B.shape
    ...
    if P0 is None:
        ...
        Q0 = np.zeros((n, n))
        R0 = np.zeros((m, m))
        K0 = _policy_fitting_with_a_kalman_constraint(
            L, r, xs, us_observed, A, B, P0, Q0, R0, niter=niter, rho=rho)

    best_P = P0
    best_Q = Q0
    best_R = R0
    best_K = K0
    best_L = evaluate_L(best_K)
    ...
    for _ in range(n_random):
        ...
        Q = 1./np.sqrt(n)*np.random.randn(n, n)
        R = 1./np.sqrt(m)*np.random.randn(m, m)
        K, P, Q, R = _policy_fitting_with_a_kalman_constraint_extra(L, r, xs, us_observed, A, B, P.T@P, Q.T@Q, R.T@R, niter=niter,
                                                     rho=rho)
        L_K = evaluate_L(K)
        best_K = best_K + K
        best_P += P
...
#             best_K = K
    ...
    return best_K / (n_random + 1), best_P / (n_random + 1), best_Q / (n_random + 1), best_R / (n_random + 1)


def policy_fitting_with_a_kalman_constraint_federated_personalized_different_trajectories(L, r, xs, us_observed, A, B, n_random=5, niter=50, rho=1):
    """
    Wrapper around _policy_fitting_with_a_kalman_constraint.
    """
    n, m = B.shape
